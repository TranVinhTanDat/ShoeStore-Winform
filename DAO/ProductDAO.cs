...
namespace DAO
{
    public class ProductDAO : DataConection
    {
        ...
        public bool UpdateQuantity(string productId, int quantity)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "UPDATE Product SET Quantity = Quantity + @quantity WHERE ProductID = @idProduct";
            cmd.Connection = conn;
            cmd.Parameters.Add("@idProduct", MySqlDbType.VarString).Value = productId;
